...
max-md:absolute left-0 z-10 ${!isMenuOpen && "max-md:-translate-x-full"}`}>
      ...
      {/* Recent chats */}
      {chats.length > 0 && <p className='mt-4 text-sm text-gray-900 dark:text-gray-100'>Recent Chats</p>}
...
hover:scale-103 transition-all' >
        ...
        <div className='flex flex-col text-sm'>
          <p className='text-gray-900 dark:text-gray-100'>Credits: {user?.credits}</p>
          <p className='test-xs text-gray-400'>Purchase Credits to use Thinko</p>
        </div>
      </div>
      {/* Theme Toggle */}
...
dark:border-white/15 rounded-md cursor-pointer group'>
        ...
        <p className='flex-1 text-sm truncate text-gray-900 dark:text-gray-100'>{user ? user.name
